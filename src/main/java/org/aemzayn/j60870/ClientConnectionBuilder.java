 ...
 * A client application that wants to
 * connect to a server should first create an instance of
 * {@link ClientConnectionBuilder}. Next all the necessary
 * configuration parameters can be set. Finally the
 * {@link ClientConnectionBuilder#build()} function is called to
 * connect to the server. An instance of {@link ClientConnectionBuilder} can be
 * used to create an unlimited number of
 * connections. Changing the parameters of a {@link ClientConnectionBuilder} has
 * no affect on connections that have
 * already been created.
 *
 * <p>
 * Note that the configured lengths of the fields COT, CA and IOA have to be the
 * same for all communicating nodes in a
 * network. The default values used by {@link ClientConnectionBuilder} are those
 * most commonly used in IEC 60870-5-104
 * communication.
 * </p>
 */
public class ClientConnectionBuilder extends CommonBuilder<ClientConnectionBuilder, Connection> {
    ...
    /**
     * Creates a client connection builder that can be used to connect to the given
     * address.
     * 
     * @param address
    ...
    /**
     * Set the socket factory to used to create the socket for the connection. The
     * default is
     * {@link SocketFactory#getDefault()}. You could pass an
     * {@link SSLSocketFactory} to enable SSL.
     * 
     * @param socketFactory
     *                      the socket factory
     * @return this builder
    ...
    /**
     * Sets an implementation of the ReservedASduTypeDecoder to define supported
     * reserved ASdus
     * 
     ...
     * 
     * @param address
     *                the local address the socket is bound to, or null for any
     *                local address.
     * @param port
     *                the local port the socket is bound to or zero for a system
     *                selected free port.
     * @return this builder
     */
    ...
    /**
     * Connects to the server. The TCP/IP connection is build up and a
     * {@link Connection} object is returned that can be
     * used to communicate with the server.
     *
     * @return the {@link Connection} object that can be used to communicate with
     *         the server.
     * @throws IOException
